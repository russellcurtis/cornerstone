...
if ($viewall == "yes") {
			$nowtime = time();
			$sql_complete = "SELECT invoice_id FROM cstone_timesheet_invoice WHERE invoice_date < $nowtime $account_test ORDER BY $order_by";
			$result_complete = mysql_query($sql_complete, $conn) or die(mysql_error());
			
} else {
			$nowtime = time();
			$sql_complete = "SELECT invoice_id FROM cstone_timesheet_invoice WHERE invoice_id = $invoice_id LIMIT 1";
			$result_complete = mysql_query($sql_complete, $conn) or die(mysql_error());
}
...
// Get the relevant infomation from the Invoice Database
	...
	$sql_invoice = "SELECT * FROM cstone_timesheet_invoice WHERE invoice_id = '$invoice_id' LIMIT 1";
	...
	$invoice_ref_print = strtoupper("Invoice $invoice_ref");
...
// Account name
	...
	if ($invoice_account > 0) {
		...
		$sql_account = "SELECT * FROM cstone_account WHERE account_id = '$invoice_account' LIMIT 1";
...
// Get the relevant infomation from the Project
	...
	$sql_project = "SELECT proj_num, proj_name, proj_client_contact_id FROM cstone_projects WHERE proj_id = '$invoice_project' LIMIT 1";
...
// Get contact prefix
	...
	$contact_companyname = NULL;
		...
		$contact_address_print = $prefix_name.$contact_namefirst." ".$contact_namesecond;
		if ($contact_companyname) { $contact_address_print = $contact_address_print."\n".$contact_companyname; }
...
//Heading Titles Text
	...
	$pdf->Cell(150,6,'Description of Work Undertaken','1','','',1);
...
// Pull the corresponding results from the Invoice Item list
			$sql2 = "SELECT * FROM cstone_timesheet_invoice_item WHERE invoice_item_invoice = '$invoice_id'";
...
// Pull the corresponding results from the Expenses
			...
			$x = 40;
			$pdf->SetXY($x,$y);
			$sql3 = "SELECT * FROM cstone_timesheet_expense WHERE ts_expense_invoiced = '$invoice_id' AND ts_expense_disbursement = '1' ORDER BY ts_expense_date";
	...
	if (($invoice_vat_total - $invoice_novat_total) > 1) {
		$vat_percent = round(((($invoice_vat_total / $invoice_novat_total) * 100 ) - 100),2); }
	elseif (($ts_expense_total_vat - $ts_expense_total) > 1) {
		$vat_percent = round(((($ts_expense_total_vat / $ts_expense_total) * 100 ) - 100),2);
	}
	...
	if ($include_vat_line == 1) {
		...
		$vat_percent_print = "VAT (at ".$vat_percent."%)";
	
		$pdf->SetXY(40,$y);
		$pdf->Cell(150,5,$vat_percent_print,'1','','',1);
		$y = $y + 8;
		$pdf->SetXY($x,$y);
		...
		$y = $y + 6;
		$pdf->SetXY(40,$y);
		$pdf->Cell(150,5,'Disbursements','1','','',1);
...
// Pull the corresponding results from the Expenses (Disbursements only)
	...
	// Total
		...
		$pdf->Cell(150,6,'TOTAL NOW DUE','1','','',1);
