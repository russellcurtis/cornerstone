...
echo "<tr><th>ID</th><th>Date</th><th colspan=\"2\">Description</th><th>Value</th><th>Paid by $input <br />but not yet reimbursed</th><th>Paid by $input<br />and already reimbursed</th><th>Paid by RCKa directly</th></tr>";
}
	...
	$sql_expense = "SELECT * FROM intranet_timesheet_expense, intranet_user_details, intranet_projects WHERE proj_id = ts_expense_project AND user_id = ts_expense_user AND ts_expense_user = '$_GET[user]' AND ts_expense_date BETWEEN '$time_begin' AND '$time_end' $show_non_personal ORDER BY ts_expense_date";
	...
	if ($show_non_personal == NULL) {
		echo "<h1>Complete Expenses for $user_name_first $user_name_second, for period ".date("d M Y",$time_begin)." to ".date("d M Y",$time_end)."</h1>";
		...
		echo "<h1>Personal Expenses for $user_name_first $user_name_second, for period ".date("d M Y",$time_begin)." to ".date("d M Y",$time_end)."</h1>";
	...
	while ($array_expense = mysql_fetch_array($result_expense)) {
		...
		echo "<tr><td style=\"$style_add\">$ts_expense_id</td><td style=\"$style_add\">$ts_expense_date</td><td colspan=\"2\" style=\"$style_add\">$ts_expense_desc</td>";
		...
		echo "<tr><td class=\"null\" style=\"$style_add\">$ts_expense_id</td><td class=\"null\" style=\"$style_add; width:35px;\">$ts_expense_date</td><td class=\"null\" style=\"$style_add\">$proj_num</td><td class=\"null\" style=\"$style_add\">$ts_expense_desc</td>";
	...
	if ($total_queried > 0) {
		echo "<tr><th colspan=\"8\">Queried Items</th></tr>";
		$style_add = "background-color: #f6d1d1; color: #e24141";
		echo "<tr><td class=\"total\" colspan=\"4\" style=\"$style_add\">Total (Queried)</td><td class=\"total\" style=\"text-align: right;$style_add\">".CashFormat($total_queried)."</td><td colspan=\"3\" style=\"$style_add\"></td></tr>";
	}
	
	echo "<tr><th colspan=\"8\">Personal Items</th></tr>";
	
	echo "<tr><td class=\"total\" colspan=\"6\">Sub Total (Personal)</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col3)."</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col4)."</td></tr>";
	
		echo "<tr><td class=\"total\" colspan=\"4\">Total (Personal)</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col1 - $total_queried)."</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col2)."</td><td class=\"total\" style=\"text-align: right;\" colspan=\"3\">".CashFormat($col3 + $col4)."</td></tr>";
		
		if ($show_non_personal == NULL) {
		
		// Non P11d Items
		
		echo "<tr><th colspan=\"8\">Non-Personal Items</th></tr>";
		
			echo "<tr><td class=\"total\" colspan=\"6\">Sub Total (non-Personal)</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col_alt_3)."</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col_alt_4)."</td></tr>";
	
		echo "<tr><td class=\"total\" colspan=\"4\">Total (non-Personal)</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col_alt_1)."</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($col_alt_2)."</td><td class=\"total\" style=\"text-align: right;\" colspan=\"3\">".CashFormat($col_alt_3 + $col_alt_4)."</td></tr>";
		...
		echo "<tr><th colspan=\"8\">Reimbursements Only</th></tr>";
		
		$total_col2 = $col2 + $col_alt_2;
		$total_col3 = $col3 + $col_alt_3;
	
		echo "<tr><td class=\"total\" colspan=\"5\">Total (Reimbursements)</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($total_col2)."</td><td class=\"total\" style=\"text-align: right;\">".CashFormat($total_col3)."</td><td></td></tr>";
...
echo "<h1>Summary</h1><p style=\"font-size: 13pt\">In financial year beginning ".date("j M Y",$time_begin).", $user_name_first $user_name_second has <strong>".CashFormat($col1 - $total_queried)."</strong> of personal expenses, <strong>".CashFormat($col3 + $col4)."</strong> of which has already been paid by RCKa, leaving <strong>".CashFormat($col2)."</strong> outstanding to be reimbursed.";
